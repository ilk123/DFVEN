...
def create_lmdb(dataset, raw_dir, lmdb_dir, filter_file=''):
    assert dataset in ('VimeoTecoGAN', 'Endoscope', 'Inter4K'), \
    ...
    for b, seq_dir in enumerate(seq_dir_lst):
        ...
        # read frames
        for i in range(n_frm // tempo_range):
            
            frm = cv2.imread(frm_path_lst[i*tempo_range], cv2.IMREAD_UNCHANGED)
            frm = np.ascontiguousarray(frm[..., ::-1])  # hwc|rgb|uint8

            h, w, c = frm.shape

            key = '{}_{}x{}x{}_{:04d}'.format(seq_dir, 1, h, w, i)
            print(key)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='Endoscope', 
                        help='VimeoTecoGAN or Endoscope')
    parser.add_argument('--data_type', type=str, default='GT_Single', 
    ...
    # setup
    raw_dir = 'E:/PELD/enhancing/data/{}/frame'.format(args.dataset)
    lmdb_dir = 'E:/PELD/enhancing/data/{}/{}.lmdb'.format(args.dataset, args.data_type)
    filter_file = ''
