...
class DfnetFour(nn.Module):
    ...
    def forward(self, lr):
        ...
        degrade_fea4, logits4, labels4 = self.Dnet4(query_patch, key_patch, is_train=True)

        degrade_fea = torch.concat((degrade_fea1, degrade_fea2, degrade_fea3, degrade_fea4), dim=1)
    ...
    def infer(self, lr):
        ...
        degrade_fea3 = self.Dnet3(x, x, is_train=False)
        degrade_fea4 = self.Dnet4(x, x, is_train=False)
        degrade_fea = torch.concat((degrade_fea1, degrade_fea2, degrade_fea3, degrade_fea4), dim=1)
